...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        ##############################################################################################
        self.db_info_path_DALI = '/space/userfiles/khatouna/SAM_IV_conference/data/DALI/voxel-rcnn/RC_PPCG_2023_snow/'
        self.db_infos_DALI = {}
        for class_name in class_names:
        ...
        ##############################################################################################
        ##############################################################################################
        self.db_info_path_LISA = '/space/userfiles/khatouna/SAM_IV_conference/data/LISA/snow_10_mm_hr/'
        self.db_infos_LISA = {}
        for class_name in class_names:
    ...
    def filter_by_max_points(self, db_infos, pts_thresh=50):
        for name in self.class_names:
            # if name == 'Car':
            # if name in db_infos.keys():
                filtered_infos = []
                for info in db_infos[name]:
                    if info['num_points_in_gt'] <= pts_thresh:
                        filtered_infos.append(info)

                if self.logger is not None:
                    self.logger.info('Database filter by max points %s: %d => %d' %
                                     (name, len(db_infos[name]), len(filtered_infos)))
                db_infos[name] = filtered_infos
